...
export default function VercelPokerScoring() {
  ...
  useEffect(() => {
    ...
    const pollForUpdates = async () => {
      try {
        const response = await fetch(`/api/sessions/${session.id}?playerId=${currentPlayerId}&lastUpdate=${lastUpdateRef.current}`)
        if (response.ok) {
  ...
  const handleUpdateStory = async (story: string) => {
    ...
    try {
      await fetch('/api/sessions/update-story', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          sessionId: session.id,
      ...
      <div className="max-w-2xl mx-auto p-6 space-y-6">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">
            Real-time collaborative story point estimation (Vercel Compatible)
          </p>
        </div>
